...
#########################################################
            ...
            colors = ['#B80000' if name != 'Benign' else '#ffba08' for name in attack]
            ...
            multiclass_fig.update_layout(
                title=dict(text='Distribution of Diverse Attack Simulation', x=0.2, y=1),
    ...
    with col2:
        ...
        with col2d:
            st.markdown("""
                        <h1 style='text-align: center; color: white; font-size: 16px; margin-top: -25px;'>
                        Workflow with Weighted Residuals and Regularization
                        </h1>
                        """, unsafe_allow_html=True)    
...
if st.session_state.page == "Binary Classification":
    ...
    st.markdown("""
    - **Purpose**: To simulate XGBoost binary classification models prediction on new, unseen datasets
    ...
    - Dataset for testing available on GitHub: https://github.com/FarihaAnis/An-IoT-Network-Intrusion-Detection-and-Classification-System-with-XGBoost-using-CICIoT2023-Dataset/blob/main/prediction_data.csv
    """)
    ...
    def preprocess_and_predict(data, scaler_path, model_path, model_type):
        ...
        model = xgb.XGBClassifier()  # Use the unpacking operator to set parameters
        ...
        df_with_predictions['Predicted Class'] = predicted_labels
    ...
    if st.session_state.uploaded_file_data is not None:
        ...
        with tab_all_features:
            ...
            with mid_col:
                ...
                # Multi-select filter for "Predicted Class"
                label_options = df_predictions['Predicted Class'].unique().tolist()
                selected_labels = st.multiselect("Filter by Predicted Class", label_options, default=label_options,  key="all_features_filter")
                # Filter the DataFrame based on the selected labels
                filtered_df = df_predictions[df_predictions['Predicted Class'].isin(selected_labels)]
                ...
                with mid_col2a:
                    ...
                    counts = list(label_values.values())
                ...
                with mid_col2b:
                    ...
                    label_counts = filtered_df['Predicted Class'].value_counts().sort_index(ascending=False)
                    ...
                    fig.update_layout(
                        title=dict(text='Distribution of Predicted Class', x=0.5, xanchor='center', y=1),
                        uniformtext_minsize=20,
                        uniformtext_mode='hide',
                        legend=dict(
                            orientation='h',
            ...
            with col2:
                ...
                st.markdown(
                    """
                    <h1 style="text-align: center; color: #ffffff; font-size: 20px; margin-bottom: 0px;">Performance Metrics:</h1>
                    """, unsafe_allow_html=True
                )
                ...
                with col1:
                    display_donut_chart("Accuracy", metrics["Accuracy"], max_value=100, color='#e8e8e8')
                    display_donut_chart("Precision", metrics["Precision"], max_value=100, color='#e8e8e8')

                # Display F1-Score and Recall in the second column
                with col2:
                    display_donut_chart("F1-Score", metrics["F1-Score"], max_value=100, color='#e8e8e8')
                    display_donut_chart("Recall", metrics["Recall"], max_value=100, color='#e8e8e8')
        ...
        with tab_rf_25:
            ...
            with mid_col:
                ...
                # Multi-select filter for "Predicted Class"
                label_options = df_predictions['Predicted Class'].unique().tolist()
                selected_labels = st.multiselect("Filter by Predicted Class", label_options, default=label_options,  key="rf_25_filter")
                # Filter the DataFrame based on the selected labels
                filtered_df = df_predictions[df_predictions['Predicted Class'].isin(selected_labels)]
                # Display the filtered DataFrame
                st.markdown("<h1 style='text-align: center; color: #ffffff; font-size: 20px; margin-top:-20px;'>Prediction Results:</h1>", unsafe_allow_html=True)
        ...
        with tab_et_25:
            ...
            with mid_col:
                ...
                # Multi-select filter for "Predicted Class"
                label_options = df_predictions['Predicted Class'].unique().tolist()
                selected_labels = st.multiselect("Filter by Predicted Class", label_options, default=label_options,  key="et_25_filter")
                # Filter the DataFrame based on the selected labels
                filtered_df = df_predictions[df_predictions['Predicted Class'].isin(selected_labels)]
                # Display the filtered DataFrame
                st.markdown("<h1 style='text-align: center; color: #ffffff; font-size: 20px; margin-top:-20px;'>Prediction Results:</h1>", unsafe_allow_html=True)
...
if st.session_state.page == "Multiclass Classification":
    ...
    st.markdown("""
    - **Purpose**: To simulate XGBoost multiclass classification models prediction on new, unseen datasets.
    ...
    if st.session_state.uploaded_file_data is not None:
        ...
        with tab_all_features:
            ...
            with mid_col:
                ...
                # Multi-select filter for "Predicted Class"
                label_options = df_predictions['Predicted Class'].unique().tolist()
                selected_labels = st.multiselect("Filter by Predicted Class", label_options, default=label_options,  key="allfeatures")
                # Filter the DataFrame based on the selected labels
                filtered_df = df_predictions[df_predictions['Predicted Class'].isin(selected_labels)]
                # Display the filtered DataFrame
                ...
                with mid_col2b:
                    ...
                    label_counts = filtered_df['Predicted Class'].value_counts().sort_values(ascending=True)
                    labels = label_counts.index.tolist()
                    counts = label_counts.values.tolist()
                    ...
                    # New color palette
                    bar_colors = ['#B80000' if label != 'benign' else '#ffba08' for label in labels]
        ...
        with tab_rf_25:
            ...
            with mid_col:
                ...
                # Multi-select filter for "Predicted Class"
                label_options = df_predictions['Predicted Class'].unique().tolist()
                selected_labels = st.multiselect("Filter by Predicted Class", label_options, default=label_options,  key="rf_25_multi")
                # Filter the DataFrame based on the selected labels
                filtered_df = df_predictions[df_predictions['Predicted Class'].isin(selected_labels)]
                # Display the filtered DataFrame
                st.markdown("<h1 style='text-align: center; color: #ffffff; font-size: 20px;'>Prediction Results:</h1>", unsafe_allow_html=True)
        ...
        with tab_et_25:
            ...
            with mid_col:
                ...
                # Multi-select filter for "Predicted Class"
                label_options = df_predictions['Predicted Class'].unique().tolist()
                selected_labels = st.multiselect("Filter by Predicted Class", label_options, default=label_options,  key="et_25_multi")
                # Filter the DataFrame based on the selected labels
                filtered_df = df_predictions[df_predictions['Predicted Class'].isin(selected_labels)]
                # Display the filtered DataFrame
                st.markdown("<h1 style='text-align: center; color: #ffffff; font-size: 20px;'>Prediction Results:</h1>", unsafe_allow_html=True)
